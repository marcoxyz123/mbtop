...
namespace Mem {
	...
	string draw(const mem_info& mem, bool force_redraw, bool data_same) {
		...
		if (redraw) {
			...
			//? Mem graphs and meters - create with per-item heights/widths for layout
			{
				...
				if (horizontal_mem_layout) {
					//? Recalculate item widths based on ACTUAL item count (may differ from calcSizes if mode changed)
					const int available_width = mem_width - 1;
					mem_item_width = available_width / num_items;
					mem_item_width_remainder = available_width % num_items;

					//? Horizontal mode: graphs use per-item width and full height
					int current_x = 0;
					int idx = 0;
					for (const auto& name : all_mem_names) {
						int item_width;
						if (idx == num_items - 1) {
							//? Last item fills remaining space to right edge (no divider after it)
							item_width = available_width - current_x;
						} else {
							//? Distribute remainder: last N-1 items before the final one get +1 width
							const int width_extra_threshold = (num_items - 1) - mem_item_width_remainder;
							item_width = mem_item_width + ((idx >= width_extra_threshold and mem_item_width_remainder > 0) ? 1 : 0);
						}
						//? Graph fills entire item width (last item has no divider, others: graph fills to divider)
						const int item_graph_width = max(1, (idx == num_items - 1) ? item_width : item_width - 1);
		...
		if (horizontal_mem_layout) {
			...
			const int available_width = mem_width - 1;
			const int base_item_width = available_width / num_mem_items;
			const int width_remainder = available_width % num_mem_items;
			...
			for (const auto& name : comb_names) {
				//? Calculate item width - last item fills to right edge
				int this_item_width;
				if (item_index == num_mem_items - 1) {
					//? Last item fills remaining space (no divider after it)
					this_item_width = available_width - current_x;
				} else {
					//? Distribute remainder among middle items
					const int width_extra_threshold = (num_mem_items - 1) - width_remainder;
				...
				//? Line 2: Value and percentage (with hot theme gradient coloring)
				out += Mv::to(y+cy+1, x+cx) + humanized + " " + Theme::g("cpu").at(clamp(percent_value, 0ll, 100ll)) + pct_str + Theme::c("main_fg");

				//? Line 3+: Graph/meter - graph handles multi-line rendering internally
				if (graph_height > 0 and not graphics.empty()) {
					out += Mv::to(y+cy+2, x+cx) + graphics;
				}

				//? Draw vertical divider between items (except after last)
				//? Divider spans: title row + value row + graph rows
				if (item_index < num_mem_items - 1) {
					out += Theme::c("div_line");
					for (int row = 0; row < graph_height + 2; row++) {
						out += Mv::to(y+cy+row, x+cx+this_item_width-1) + Symbols::v_line;
					}
					out += Theme::c("main_fg");
				}

				current_x += this_item_width;
				cx += this_item_width;
				item_index++;
			}
		}
		...
		if (show_disks) {
			...
			string disk_divider = Mv::l(1) + Theme::c("div_line") + Symbols::div_left + Symbols::h_line * disks_width + Theme::c("mem_box") + Fx::ub + Symbols::div_right + Mv::l(disks_width);
			...
			if (io_mode) {
				...
				for (const auto& mount : mem.disks_order) {
					...
					const string title_color = is_selected ? Theme::c("hi_fg") : Theme::c("title");
					out += Mv::to(y+1+cy, x+1+cx) + disk_divider + title_color + Fx::b + uresize(disk.name, disks_width - 8) + Mv::to(y+1+cy, x+cx + disks_width - total.size())
						+ trans(total) + Fx::ub;
					if (big_disk) {
				...
				for (const auto& mount : mem.disks_order) {
					...
					const bool is_selected = (disk_selected > 0 and visible_index == disk_selected);
					const string title_color = is_selected ? Theme::c("hi_fg") : Theme::c("title");
					out += Mv::to(y+1+cy, x+1+cx) + disk_divider + title_color + Fx::b + uresize(disk.name, disks_width - 8) + Mv::to(y+1+cy, x+cx + disks_width - human_total.size())
...
namespace Proc {
	...
	int selection(const std::string_view cmd_key) {
		...
		if ((cmd_key == "up" or (vim_keys and cmd_key == "k")) and selected > 0) {
			...
			const int safe_divisor = max(1, select_max - 2);  //? Guard against division by zero
			start = clamp((int)round((double)mouse_y * (numpids - select_max - 2) / safe_divisor), 0, max(0, numpids - select_max));
		}
	...
	string draw(const vector<proc_info>& plist, bool force_redraw, bool data_same) {
		...
		if (redraw) {
			...
			bool show_threads_cfg = Config::getB("proc_show_threads");
			bool show_user_cfg = Config::getB("proc_show_user");
			bool show_memory_cfg = Config::getB("proc_show_memory");
			bool show_cpu_cfg = Config::getB("proc_show_cpu");
			bool show_io_cfg = Config::getB("proc_show_io");         //? Combined IO for side layout
			bool show_io_read_cfg = Config::getB("proc_show_io_read");   //? IO Read for bottom layout
			bool show_io_write_cfg = Config::getB("proc_show_io_write"); //? IO Write for bottom layout
			bool show_state_cfg = Config::getB("proc_show_state");
			bool show_priority_cfg = Config::getB("proc_show_priority");
			bool show_nice_cfg = Config::getB("proc_show_nice");
			bool show_ports_cfg = Config::getB("proc_show_ports");
			bool show_virt_cfg = Config::getB("proc_show_virt");
			bool show_runtime_cfg = Config::getB("proc_show_runtime");
			bool show_cputime_cfg = Config::getB("proc_show_cputime");
			bool show_gputime_cfg = Config::getB("proc_show_gputime");
			...
			if (bottom_layout and not proc_tree) {
				...
				//? Each column uses size + 2 spacing (only if visible)
				int fixed_cols = 8 + 2;  // Pid always visible
				if (user_size > 0) fixed_cols += user_size + 2;
				if (state_size > 0) fixed_cols += state_size + 2;
				if (priority_size > 0) fixed_cols += priority_size + 2;
				if (nice_size > 0) fixed_cols += nice_size + 2;
				if (thread_size > 0) fixed_cols += thread_size + 2;
				if (ports_size > 0) fixed_cols += ports_size + 2;
				if (io_read_size > 0) fixed_cols += io_read_size + 2 + io_write_size + 2;
				if (show_memory_cfg) fixed_cols += 5 + 2;  // Mem
				...
				if (show_cpu_cfg) fixed_cols += 5 + 2;  // Cpu% (no graph in bottom layout)
			...
			//? Defensive bounds checking for size calculations - prevent crashes during layout transitions
			...
			//? Order MUST match the visual column order in the process table
			visible_sort_fields.clear();
			visible_sort_fields.push_back("pid");
			visible_sort_fields.push_back("name");
			if (bottom_layout) {
				//? Bottom layout order: Pid | Program | User | Sta | Pri | Ni | Thr | Ports | IO/R | IO/W | Mem | Virt | CpuT | GpuT | Time | Cpu% | Gpu% | Command
				if (show_user_cfg and user_size > 0) visible_sort_fields.push_back("user");
				if (show_state_cfg) visible_sort_fields.push_back("state");
				if (show_priority_cfg) visible_sort_fields.push_back("priority");
				if (show_nice_cfg) visible_sort_fields.push_back("nice");
				if (show_threads_cfg and thread_size > 0) visible_sort_fields.push_back("threads");
				if (show_ports_cfg and ports_size > 0) visible_sort_fields.push_back("ports");
				if (show_io_read_cfg and io_read_size > 0) visible_sort_fields.push_back("io read");
				if (show_io_write_cfg and io_write_size > 0) visible_sort_fields.push_back("io write");
				if (show_memory_cfg) visible_sort_fields.push_back("memory");
				if (show_virt_cfg and virt_size > 0) visible_sort_fields.push_back("virt mem");
				if (show_cputime_cfg and cpu_time_size > 0) visible_sort_fields.push_back("cpu time");
				if (show_gputime_cfg and gpu_time_size > 0) visible_sort_fields.push_back("gpu time");
				if (show_runtime_cfg and runtime_size > 0) visible_sort_fields.push_back("runtime");
				if (show_cpu_cfg) {
					visible_sort_fields.push_back("cpu direct");
					visible_sort_fields.push_back("cpu lazy");
				}
				if (show_gpu) visible_sort_fields.push_back("gpu");
				if (show_cmd and cmd_size > 0) visible_sort_fields.push_back("command");
			}
			else {
				//? Side layout order: Pid | Program | Command | Threads | User | Mem | IO | Cpu% | Gpu%
				if (show_cmd and cmd_size > 0) visible_sort_fields.push_back("command");
				if (show_threads_cfg and thread_size > 0) visible_sort_fields.push_back("threads");
				if (show_user_cfg and user_size > 0) visible_sort_fields.push_back("user");
				if (show_memory_cfg) visible_sort_fields.push_back("memory");
				if (show_io_cfg and io_size > 0) visible_sort_fields.push_back("io");
				if (show_cpu_cfg) {
					visible_sort_fields.push_back("cpu direct");
					visible_sort_fields.push_back("cpu lazy");
				}
			...
			if (show_detailed) {
				...
				//? Defensive bounds checking for detailed box dimensions - skip if invalid
				dgraph_width = max(10, dgraph_width);
				d_width = max(10, d_width);
				if (d_x < 0 or d_y < 0 or dgraph_x < 0 or dgraph_width > width or d_width > width) {
					//? Dimensions invalid during layout transition, skip detailed rendering
					goto skip_detailed_redraw;
				}

				//? Create cpu, gpu and mem graphs if process is alive
				if (alive or pause_proc_list) {
					//? Non-linear scaling for better visibility of low values
					//? Height=3 (12 dots): ~8.33% per dot
					auto scale_to_12 = [](double pct) -> long long {
						if (pct < 0.1) return 0;
						if (pct <= 8.33) return 8;
						if (pct <= 16.67) return 16;
						if (pct <= 25) return 25;
						if (pct <= 33.33) return 33;
						if (pct <= 41.67) return 41;
						if (pct <= 50) return 50;
						if (pct <= 58.33) return 58;
						if (pct <= 66.67) return 66;
						if (pct <= 75) return 75;
						if (pct <= 83.33) return 83;
						if (pct <= 91.67) return 91;
						return 100;
					};

					//? Scale CPU percent for graph
					deque<long long> scaled_cpu;
					for (const auto& val : detailed.cpu_percent)
						scaled_cpu.push_back(scale_to_12(static_cast<double>(val)));
					detailed_cpu_graph = Draw::Graph{dgraph_width - 1, 3, "cpu", scaled_cpu, graph_symbol, false, true, 100};

					//? Scale GPU percent for graph
					deque<long long> scaled_gpu;
					for (const auto& val : detailed.gpu_percent)
						scaled_gpu.push_back(scale_to_12(static_cast<double>(val)));
					detailed_gpu_graph = Draw::Graph{dgraph_width - 1, 3, "cpu", scaled_gpu, graph_symbol, false, true, 100};

					//? Memory graph (original linear scaling)
				...
				//? Command line (highlighted to indicate clickable)
			...
			if (not proc_tree) {
				if (bottom_layout) {
					...
					//? Pid column (sortable) - always shown
					add_header("Pid:", 8, "pid");
					//? Program column (sortable by name) - always shown
					add_header("Program:", prog_size, "name", false);
					//? User column (sortable, conditional)
					if (user_size > 0) add_header("User:", user_size, "user", false);
					//? State column (sortable, conditional)
					if (state_size > 0) add_header("Sta", state_size, "state");
					//? Priority column (sortable, conditional)
					if (priority_size > 0) add_header("Pri", priority_size, "priority");
					//? Nice column (sortable, conditional)
					if (nice_size > 0) add_header("Ni", nice_size, "nice");
					//? Threads column (sortable, conditional)
					if (thread_size > 0) add_header("Thr", thread_size, "threads");
					...
					//? Memory column (sortable, conditional)
					if (show_memory_cfg) add_header((mem_bytes ? "MemB" : "Mem%"), 5, "memory");
					...
					//? CPU% column (sortable, conditional) - use "cpu direct" since we display instant cpu_p value
					if (show_cpu_cfg) add_header("Cpu%", 5, "cpu direct");
					//? GPU% column (sortable, conditional)
					if (show_gpu) {
						add_header("Gpu%", 5, "gpu");
					}
					//? Command column (not sortable per user request, conditional)
					if (cmd_size > 0) {
					...
					//? Headers only shown if config enabled and size > 0
					...
					out += (thread_size > 0 ? Mv::l(4) + "Threads: " : "")
						+ (user_size > 0 ? ljust("User:", user_size) + ' ' : "")
						+ (show_memory_cfg ? rjust((mem_bytes ? "MemB" : "Mem%"), 5) + ' ' : "")
						+ (io_size > 0 ? rjust("IO", io_size) + ' ' : "")
						+ (show_cpu_cfg ? rjust("Cpu%", (show_graphs ? 10 : 5)) : "")
		...
		if (show_detailed) {
			...
			if (d_width < 10 or dgraph_width < 10 or d_x < 0 or d_y < 0) goto skip_detailed_draw;

			bool alive = detailed.status != "Dead";
			const int item_fit = max(1, (int)floor((double)(d_width - 2) / 10));
			const int item_width = max(1, (int)floor((double)(d_width - 2) / min(item_fit, 8)));

			//? Non-linear scaling for graph updates (height=3, 12 dots)
			auto scale_to_12_update = [](double pct) -> long long {
				if (pct < 0.1) return 0;
				if (pct <= 8.33) return 8;
				if (pct <= 16.67) return 16;
				if (pct <= 25) return 25;
			...
			deque<long long> scaled_cpu_update;
			for (const auto& val : detailed.cpu_percent)
				scaled_cpu_update.push_back(scale_to_12_update(static_cast<double>(val)));
			out += Mv::to(d_y + 1, dgraph_x + 1) + Fx::ub + detailed_cpu_graph(scaled_cpu_update, (redraw or data_same or not alive))
			...
			deque<long long> scaled_gpu_update;
			for (const auto& val : detailed.gpu_percent)
				scaled_gpu_update.push_back(scale_to_12_update(static_cast<double>(val)));
			out += Mv::to(d_y + 5, dgraph_x + 1) + Fx::ub + detailed_gpu_graph(scaled_gpu_update, (redraw or data_same or not alive))
			...
			out += Mv::to(d_y + 4, d_x + 1) + Theme::c("title") + Fx::b + rjust((item_fit > 4 ? "Memory: " : "M:") + rjust(mem_str, 4) + "% ", max(1, (d_width / 3) - 2))
		...
		//? Read column visibility config settings for rendering loop
		const bool render_show_memory = Config::getB("proc_show_memory");
		const bool render_show_cpu = Config::getB("proc_show_cpu");
		...
		for (int n=0; auto& p : plist) {
			...
			if (is_selected or is_followed) {
				...
				const int safe_select_max = max(1, select_max);  //? Guard against division by zero
				if (proc_colors) {
					...
					for (int i = 0; int v : {(int)round(p.cpu_p), (int)round(p.mem * 100 / totalMem), (int)p.threads / 3}) {
						if (proc_gradient) {
							int val = (min(v, 100) + 100) - calc * 100 / safe_select_max;
					...
					if (show_gpu) {
						...
						if (proc_gradient) {
							int val = (min(gpu_v, 100) + 100) - calc * 100 / safe_select_max;
				...
				if (proc_gradient) {
					g_color = Theme::g("proc").at(clamp(calc * 100 / safe_select_max, 0, 100));
				}
			}
			...
			if (bottom_layout and not proc_tree) {
				...
				//? Columns only shown if config enabled and size > 0
				...
				out += (user_size > 0 ? g_color + ljust((cmp_greater(p.user.size(), user_size) ? p.user.substr(0, user_size - 1) + '+' : p.user), user_size) + "  " : "")
					+ (state_size > 0 ? rjust(state_str, state_size) + "  " : "")
					+ (priority_size > 0 ? rjust(to_string(p.p_priority), priority_size) + "  " : "")
					+ (nice_size > 0 ? rjust(nice_str, nice_size) + "  " : "")
					...
					+ (render_show_memory ? m_color + rjust(mem_str, 5) + "  " + end : "")
				...
				out += (thread_size > 0 ? t_color + rjust(proc_threads_string, thread_size) + ' ' + end : "" )
					+ (user_size > 0 ? g_color + ljust((cmp_greater(p.user.size(), user_size) ? p.user.substr(0, user_size - 1) + '+' : p.user), user_size) + ' ' : "")
					+ (render_show_memory ? m_color + rjust(mem_str, 5) + end + ' ' : "")
					+ (io_size > 0 ? g_color + rjust(io_str, io_size) + ' ' + end : "")
					+ (render_show_cpu ? (is_selected or is_followed ? "" : Theme::c("inactive_fg")) + (show_graphs ? graph_bg * 5: "")
	...
	void calcSizes() {
		...
		if (Mem::shown) {
			...
			if (show_disks) {
				...
				divider = x + mem_width;
				horizontal_mem_layout = false;
				mem_item_width = 0;
				mem_item_width_remainder = 0;
			}
			else {
				mem_width = width - 1;
			...
			if (horizontal_mem_layout) {
				//? In horizontal mode, use per-item width for meter/graph width
			...
			if (need_multiline_graphs) {
				if (horizontal_mem_layout) {
					...
					//? No swap_overhead: swap items are displayed in columns, not extra rows
					int available_graph_lines = (height - 2) - 1 - 2;
					graph_height = max(1, available_graph_lines);
					graph_height_remainder = 0;  //? No remainder needed - all same height
				}
				else {
